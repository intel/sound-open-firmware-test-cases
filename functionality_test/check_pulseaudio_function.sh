...
for samplerate in ${inputsample[*]};do
	for bit in ${bitlist[*]}; do
		...
		if [ $? != 0 ]; then
			...
			echo "Check_Media_Playback_Pipeline_Playback_"$samplerate"_Bit_"$bit" FAIL" >> playback.log
		else
			echo "Check_Media_Playback_Pipeline_Playback_"$samplerate"_Bit_"$bit" PASS" >> playback.log
		fi
	done
done
...
for bit in S16_LE S24_LE S32_LE ; do
	...
	if [ $? != 0 ]; then
		...
		echo "Check_Low_Latency_Pipeline_Playback_48000_Bit_"$bit" FAIL" >> playback.log
	else
		echo "Check_Low_latency_Pipeline_Playback_48000_Bit_"$bit" PASS" >> playback.log
	fi
done
